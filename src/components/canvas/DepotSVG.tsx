...
export const DepotSVG = () => {
  ...
  return (
    ...
    >
      <defs>
        {/* Crosshatch pattern for offline stalls */}
      ...
      {/* Directional arrows - left aisle (up) */}
      {[60, 100, 140, 180].map((y) => (
        <polygon key={`au${y}`} points={`30,${y} 28,${y + 5} 32,${y + 5}`} fill="#ffffff" opacity={0.15} />
      ))}
      {/* Directional arrows - right aisle (down) */}
      {[60, 100, 140, 180].map((y) => (
        <polygon key={`ad${y}`} points={`275,${y + 5} 273,${y} 277,${y}`} fill="#ffffff" opacity={0.15} />
      ))}

      {/* Solar canopy dashed outline over L2 field */}
      <rect
        x={40} y={75} width={220} height={80}
        fill="none" stroke="#00B4A6" strokeWidth={0.4} strokeDasharray="4,2" opacity={0.3}
      />
