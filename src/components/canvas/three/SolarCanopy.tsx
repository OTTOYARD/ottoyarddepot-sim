  ...
  return (
    <group position={[0, 0, 20]}>
      ...
      <mesh position={[0, h, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#586878" roughness={0.72} metalness={0.08} />
      </mesh>
      ...
      {Array.from({ length: Math.floor(w / 16) }, (_, i) => (
        <group key={`panel${i}`}>
          <mesh position={[-w / 2 + 8 + i * 16, h + 0.22, 0]}>
            ...
            <meshStandardMaterial color="#2c3c4e" roughness={0.55} metalness={0.05} />
          </mesh>
          {/* Aluminum edge frame */}
          <mesh position={[-w / 2 + 8 + i * 16, h + 0.22, (d - 4) / 2]}>
            <boxGeometry args={[14.8, 0.1, 0.15]} />
            <meshStandardMaterial color="#aab0b6" roughness={0.5} metalness={0.15} />
          </mesh>
          <mesh position={[-w / 2 + 8 + i * 16, h + 0.22, -(d - 4) / 2]}>
            <boxGeometry args={[14.8, 0.1, 0.15]} />
            <meshStandardMaterial color="#aab0b6" roughness={0.5} metalness={0.15} />
      ...
      {Array.from({ length: cols }, (_, i) => {
        ...
        return [d / 2 - 5, -d / 2 + 5].map((z, j) => (
          <mesh key={`c${i}${j}`} position={[x, h / 2, z]} castShadow>
            ...
            <meshStandardMaterial color="#888e94" roughness={0.68} metalness={0.1} />
          </mesh>
        ));
      })}
