...
| Copilot failed with a generic non-2xx error. | The core project's function log recorded HTTP 502 for the browser audit call. A later direct invocation of the same run returned HTTP 200 with a summary of 80 decisions, so the failure was intermittent. | Read the function's structured error response and preserve its measured decision summary when present. Do not present model analysis as successful on a failed call. |
...
The live browser validated the merged baseline and revealed these defects. The fixes are on a new branch and were validated by focused regression tests, typecheck, full Vitest suite, and production build. The cloud browser lacks WebGL, so it cannot certify 3D placement or motion even on an accessible site. The new recovery UI needs a deployed preview or merge to verify visually in the browser. Copilot succeeded on a retry, but its intermittent 502 is not fixed at the provider or function layer; this PR improves the honest failure state and keeps measured summary data visible when supplied by the function.
